...
#define ARCADEBOT_WEAPON_COEFF      1.3f
...
{
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_PLASMA)) return 0;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_VOLCANO)) return 1;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_HOMING_MISSILE)) return 2;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_BOMB)) return 3;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_FLAMETHROWER)) return 4;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_BIGBOOM)) return 5;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_LIGHTENING)) return 6;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_LASER)) return 7;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_GUN)) return 8;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_ABLAZE)) return 9;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_SHORTED)) return 10;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_DEBRIS)) return 11;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_REPAIR)) return 12;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON0)) return 13;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON1)) return 14;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON2)) return 15;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON3)) return 16;
    return -1;
}

__forceinline  EWeapon WeapName2Weap(const wchar *w)
{
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_PLASMA)) return WEAPON_PLASMA;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_VOLCANO)) return WEAPON_VOLCANO;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_HOMING_MISSILE)) return WEAPON_HOMING_MISSILE;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_BOMB)) return WEAPON_BOMB;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_FLAMETHROWER)) return WEAPON_FLAMETHROWER;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_BIGBOOM)) return WEAPON_BIGBOOM;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_LIGHTENING)) return WEAPON_LIGHTENING;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_LASER)) return WEAPON_LASER;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_GUN)) return WEAPON_GUN;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_REPAIR)) return WEAPON_REPAIR;
    ...
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON0)) return WEAPON_CANNON0;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON1)) return WEAPON_CANNON1;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON2)) return WEAPON_CANNON2;
    if (WStrCmp(w,PAR_SOURCE_DAMAGES_WEAPON_CANNON3)) return WEAPON_CANNON3;
    return WEAPON_NONE;
}

__forceinline int Weap2Index(EWeapon w)
{
    if (w==WEAPON_PLASMA) return 0;
    if (w==WEAPON_VOLCANO) return 1;
    if (w==WEAPON_HOMING_MISSILE) return 2;
    if (w==WEAPON_BOMB) return 3;
    if (w==WEAPON_FLAMETHROWER) return 4;
    if (w==WEAPON_BIGBOOM) return 5;
    if (w==WEAPON_LIGHTENING) return 6;
    if (w==WEAPON_LASER) return 7;
    if (w==WEAPON_GUN) return 8;
    if (w==WEAPON_ABLAZE) return 9;
    if (w==WEAPON_SHORTED) return 10;
    if (w==WEAPON_DEBRIS) return 11;
    if (w==WEAPON_REPAIR) return 12;
    if (w==WEAPON_CANNON0) return 13;
    if (w==WEAPON_CANNON1) return 14;
    if (w==WEAPON_CANNON2) return 15;
    if (w==WEAPON_CANNON3) return 16;
    return -1;
}
...
class CMatrixEffectWeapon : public CMatrixEffect
{
    ...
    CMatrixEffectWeapon(const D3DXVECTOR3 &start, const D3DXVECTOR3 &dir, DWORD user, FIRE_END_HANDLER handler, EWeapon type, int cooldown);
