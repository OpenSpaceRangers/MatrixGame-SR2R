...
void CFormMatrixGame::Takt(int step)
{
    ...
    {
        CreateDirectory(FOLDER_NAME_SCREENSHOTS, NULL);
        ...
        CStr    n(FOLDER_NAME_SCREENSHOTS,g_CacheHeap);
        n += "\\";
        ...
        n.Insert(0, "\\", 1);
        n.Insert(0,FOLDER_NAME_SCREENSHOTS);
        n.Add(".png",4);
        ...
        g_MatrixMap->m_DI.T(L"Screen shot has saved...", L"");
...
void CFormMatrixGame::MouseMove(int x,int y)
{
    ...
    if (g_MatrixMap->IsMouseCam())
    {
        ...
        if (p_side->GetArcadedObject())
        {
            ...
            if (dx > 0)
                p_side->GetArcadedObject()->AsRobot()->RotateRobotRite();
        }
...
void CFormMatrixGame::MouseKey(ButtonStatus status,int key,int x,int y)
{
    ...
    if (status == B_WHEEL)
    {
        while (key > 0)
        {
            g_MatrixMap->m_Camera.RotUpStep();
            --key;
        }
        while (key < 0)
        {
            g_MatrixMap->m_Camera.RotDownStep();
            ++key;
        }
