...
{
    for (int i=1; i<m_CoordCount; ++i)
        if (m_Coord[i].present) Draw(m_Coord + i,pbd);
}
...
void CMatrixProgressBar::Draw(SPBPos *pos, bool pbd)
{
    ...
    if ( (Float2Int(pos->m_X)  >= g_ScreenX) ||
         (Float2Int(pos->m_Y + h) < 0) ||
          (Float2Int(pos->m_X + pos->m_Width) < 0) ||
         (Float2Int(pos->m_Y)  >= g_ScreenY) ) return;
    ...
    p[0].tu = 0;    p[0].tv = 0.25f;
    p[1].tu = 0;    p[1].tv = 0;
    p[2].tu = 0.25; p[2].tv = 0.25f;
    p[3].tu = 0.25; p[3].tv = 0;
    p[4].tu = 0.75; p[4].tv = 0.25;
    p[5].tu = 0.75; p[5].tv = 0;
    p[6].tu = 1;    p[6].tv = 0.25;
    p[7].tu = 1;    p[7].tv = 0;
    ...
    } else
    {
        ...
        DWORD c;
        if (m_Pos < 0.5f) c = LIC(PB_COLOR_0, PB_COLOR_1,  m_Pos * 2.0f); else
        ...
        if (m_Pos < pp)
        {
            ...
            {
                p[0].tv = 1;     p[0].tu = 0.5;
                p[1].tv = 0.75f; p[1].tu = 0.5;
                p[2].tv = 1;     p[2].tu = 1;
                p[3].tv = 0.75f; p[3].tu = 1;
            } else if (i == 2)
            {
                p[0].tv = 1;     p[0].tu = 0;
                p[1].tv = 0.75f; p[1].tu = 0;
                p[2].tv = 1;     p[2].tu = 0.5f;
                p[3].tv = 0.75f; p[3].tu = 0.5f;
            } else if (i == 3)
            {
                p[0].tv = 0.75f; p[0].tu = 0.5;
                p[1].tv = 0.5f;  p[1].tu = 0.5;
                p[2].tv = 0.75f; p[2].tu = 1;
                p[3].tv = 0.5f;  p[3].tu = 1;
            } else if (i >= 4)
            {
                p[0].tv = 0.75f; p[0].tu = 0;
                p[1].tv = 0.5f;  p[1].tu = 0;
                p[2].tv = 0.75f; p[2].tu = 0.5f;
                p[3].tv = 0.5f;  p[3].tu = 0.5f;
            }
...
void CMatrixProgressBar::DrawAll(void)
{
    ...
    if (m_First == NULL && m_FirstClones == NULL) return;
...
void CMatrixProgressBar::DrawAllClones(void)
{
    ...
    if (m_FirstClones == NULL) return;

    bdi();

 //   ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MAGFILTER,			D3DTEXF_LINEAR));
	//ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MINFILTER,			D3DTEXF_NONE));
	//ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MIPFILTER,			D3DTEXF_NONE));
