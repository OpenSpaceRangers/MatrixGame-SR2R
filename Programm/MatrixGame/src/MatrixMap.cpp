...
void CMatrixMap::IdsClear()
{
	...
	if(m_Ids)
    {
		...
		HFree(m_Ids,g_MatrixHeap); m_Ids=NULL; 
	}
	m_IdsCnt=0;
}
...
void CMatrixMap::RobotPreload(void)
{
    ...
    for(int i=0;i<ROBOT_ARMOR_CNT;i++)
    {
        ...
        for(int u=0;u<n;u++)
        {
            if(vo->GetMatrixId(u) < 20) continue;
            ...
            for(int p=0;p<cntp;p++)
            {
                ...
                if(p==0 && tstr2!=L"W") break;
                else if(p==0)
                {
                    if(m_RobotWeaponMatrix[i].cnt>=ROBOT_WEAPONS_PER_ROBOT_CNT) break;
                ...
                if(tstr2.IsOnlyInt())
                {
                    if((m_RobotWeaponMatrix[i].list[m_RobotWeaponMatrix[i].cnt-1].access_invert & (~SETBIT(31))) == 0)
                    {
...
void CMatrixMap::UnitClear(void)
{
	...
	if(m_Unit!=NULL) { HFree(m_Unit,g_MatrixHeap); m_Unit=NULL; }
	if(m_Point!=NULL) { HFree(m_Point,g_MatrixHeap); m_Point=NULL; }
    if(m_Move!=NULL) { HFree(m_Move,g_MatrixHeap); m_Move=NULL; }
}
...
DWORD CMatrixMap::GetColor(float wx,float wy)
{
	...
	if((un==NULL) || (un->IsWater())) return m_AmbientColorObj;
...
float CMatrixMap::GetZ(float wx,float wy)
{
	...
	SMatrixMapUnit * un=UnitGetTest(x,y);
	if (un==NULL) return -1000.0f;
    if (!un->IsBridge()) {if (un->IsWater()) return -1000.0f;}
...
//	D3DXPlaneNormalize(&pl,&pl);
	...
	if(ox!=NULL) *ox=TruncFloat(v.x/GLOBAL_SCALE);
	if(oy!=NULL) *oy=TruncFloat(v.y/GLOBAL_SCALE);
...
*/
    ...
    {
        CMatrixMapStatic::SortEndRecalcTerainColor(); RESETFLAG(m_Flags, MMFLAG_NEEDRECALCTER);
    }
...
void CMatrixMap::DrawLandscape(bool all)
{
    ...
    ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MIPFILTER,D3DTEXF_LINEAR));
...
void CMatrixMap::Takt(int step)
{
    ...
    //            ar[i] = HNew(g_CacheHeap) CBillboard(TRACE_PARAM_CALL pos + D3DXVECTOR3(sn*r,cs*r,0), 3, a,0xFF400401, false, CMatrixEffect::GetBBTex(BBT_SMOKE));
...
bool    CMatrixMap::FindObjects(const D3DXVECTOR2 &pos, float radius, float oscale, DWORD mask, CMatrixMapStatic *skip, ENUM_OBJECTS2D callback, DWORD user)
{
#ifdef _DEBUG
    ...
    if (maxx1<minx1) {  maxx1 ^= minx1; minx1 ^= maxx1; maxx1 ^= minx1; }
    if (maxy1<miny1) {  maxy1 ^= miny1; miny1 ^= maxy1; maxy1 ^= miny1; }
    ++maxx1;
    ++maxy1;
        ...
        if (m_AD_Obj[od]->m_IntersectFlagFindObjects != m_IntersectFlagFindObjects)
        {
            ...
            for(;;)
            {
                ...
                if (ms->GetObjectType() == OBJECT_TYPE_FLYER)
                {
                    ...
                    if ((mask & TRACE_FLYER) == 0) continue;
...
bool    CMatrixMap::FindObjects(const D3DXVECTOR3 &pos, float radius, float oscale, DWORD mask, CMatrixMapStatic *skip, ENUM_OBJECTS callback, DWORD user)
{
#ifdef _DEBUG
    static int intercount = 0;
