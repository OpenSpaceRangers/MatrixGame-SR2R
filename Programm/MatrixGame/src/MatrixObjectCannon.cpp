...
CMatrixCannon::~CMatrixCannon()
{
	...
	ReleaseMe();
    UnitClear();
	if(m_ShadowProj) { HDelete(CMatrixShadowProj,m_ShadowProj,g_MatrixHeap); m_ShadowProj=NULL; }
...
void CMatrixCannon::UnitClear(void)
{
    ...
    {
        if(m_Unit[i].m_Graph) { UnloadObject(m_Unit[i].m_Graph,NULL); m_Unit[i].m_Graph=NULL; }
        ...
        {
            if(m_Unit[i].m_ShadowStencil) { HDelete(CVOShadowStencil,m_Unit[i].m_ShadowStencil,NULL); m_Unit[i].m_ShadowStencil=NULL; }
        }
    }
    ZeroMemory(m_Unit,sizeof(m_Unit));
...
void CMatrixCannon::RNeed(DWORD need)
{
	...
	if(need & m_RChange & (MR_Graph))
    {
        ...
        for(;;)
        {
            ...
            if (*nam == 'T') type = CUT_TURRET; else
            if (*nam == 'S') type = CUT_SHAFT;

            if (type == CUT_EMPTY) ERROR_E;


            m_Unit[m_UnitCnt].m_Type = type;

            m_Unit[m_UnitCnt].m_Graph = LoadObject((CWStr(OBJECT_PATH_CANNONS, g_CacheHeap) + nam + ctype + L".vo").Get(), g_MatrixHeap, true);
            ...
            if (type == CUT_SHAFT)
            {
                ...
                for (int i =0; i<n; ++i)
                {
                    ...
                    if (id >=50 && id <=59)
                    {
                        ...
                        m_Weapons[m_WeaponCnt] = (CMatrixEffectWeapon *)CMatrixEffect::CreateWeapon(D3DXVECTOR3(0,0,0), D3DXVECTOR3(0,0,1), (DWORD)this->m_Core, FireHandler, props->weapon);
    ...
    */
	if(need & m_RChange & MR_ShadowStencil) {
		...
		if(m_ShadowType!=SHADOW_STENCIL) {
			for(int i=0;i<m_UnitCnt;i++) {
				if(m_Unit[i].m_ShadowStencil) { HDelete(CVOShadowStencil,m_Unit[i].m_ShadowStencil,g_MatrixHeap); m_Unit[i].m_ShadowStencil=NULL; }
			}
		} else  {
			...
			for(int i=0;i<m_UnitCnt;i++)
            {
				...
				if(!m_Unit[i].m_ShadowStencil) m_Unit[i].m_ShadowStencil=HNew(g_MatrixHeap) CVOShadowStencil(g_MatrixHeap);
...
//					if(!(m_Unit[i].m_Graph->VO()->EdgeExist())) m_Unit[i].m_Graph->VO()->EdgeBuild();
                ...
                //D3DXPLANE cutpl;
                //D3DXPlaneTransform(&cutpl, IsNearBase()?(&g_MatrixMap->m_ShadowPlaneCutBase):(&g_MatrixMap->m_ShadowPlaneCut), &m_Unit[i].m_IMatrix);
                float len = (GetRadius()*2) +  m_Unit[i].m_Matrix._43 - (IsNearBase()?g_MatrixMap->m_GroundZBase:g_MatrixMap->m_GroundZ);
                m_Unit[i].m_ShadowStencil->Build(*(m_Unit[i].m_Graph->VO()),m_Unit[i].m_Graph->GetVOFrame(), light,len,false);
			}
		}
	}
	if(need & m_RChange & MR_ShadowProjGeom) {
		...
		} else 
        {
			...
			BYTE * buf=(BYTE *)_alloca(sizeof(CVectorObjectAnim *)*m_UnitCnt+sizeof(int)*m_UnitCnt+sizeof(D3DXMATRIX)*m_UnitCnt);
            ...
            ShadowProjBuildGeomList(*m_ShadowProj,m_UnitCnt,obj,noframe,wm,m_Core->m_Matrix, m_Core->m_IMatrix,g_MatrixMap->m_LightMain,int(100/GLOBAL_SCALE),true);
	...
	if(need & m_RChange & MR_ShadowProjTex) {
		...
		if(m_ShadowProj != NULL)
        {
			...
			ShadowProjBuildTextureList(*m_ShadowProj,m_UnitCnt,obj,tex,noframe,wm,m_Core->m_Matrix,m_Core->m_IMatrix,g_MatrixMap->m_LightMain,m_ShadowSize);
		}
	}
}
...
void CMatrixCannon::Takt(int cms)
{
	...
	for(int i=0;i<m_UnitCnt;i++) {
		if(m_Unit[i].m_Graph) {
            ...
            {
				if(m_ShadowType==SHADOW_STENCIL) RChange(MR_ShadowStencil); else
...
bool CMatrixCannon::Pick(const D3DXVECTOR3 & orig, const D3DXVECTOR3 & dir,float * outt) const
{
	...
	for(int i=0;i<m_UnitCnt;i++)
    {
		if(m_Unit[i].m_Graph)
        {
            if (m_Unit[i].m_Graph->Pick(m_Unit[i].m_Matrix, m_Unit[i].m_IMatrix,orig,dir,outt)) return true;
		}
	}
	return false;
}
...
void CMatrixCannon::FreeDynamicResources(void)
{
    ...
    } else if (m_ShadowType == SHADOW_STENCIL)
    {
        for (int i = 0; i<m_UnitCnt; ++i)
        {
            if (m_Unit[i].m_ShadowStencil) m_Unit[i].m_ShadowStencil->DX_Free();
        }
    }
...
void CMatrixCannon::BeforeDraw(void)
{
	...
	for(int i=0;i<m_UnitCnt;i++)
    {
        ...
        if (m_CurrState != CANNON_DIP && m_Unit[i].m_ShadowStencil) m_Unit[i].m_ShadowStencil->BeforeRender();
	}
...
static bool FindTarget(const D3DXVECTOR3 & center, CMatrixMapStatic *ms, DWORD user)
{
    ...
    if(match || dot > d->coss)
    {
        ...
        CMatrixMapStatic * cel = g_MatrixMap->Trace(NULL, center, ms->GetGeoCenter(), TRACE_OBJECTSPHERE|TRACE_ROBOT|TRACE_FLYER|TRACE_LANDSCAPE, d->skip);
...
void CMatrixCannon::LogicTakt(int takt)
{
    ...
    if (itstime) // время пить херши :)
    {
        ...
        g_MatrixMap->FindObjects(GetGeoCenter(), props->seek_radius, 1, TRACE_ROBOT|TRACE_FLYER, NULL, FindTarget, (DWORD)&data);
...
no_target:
    ...
    } else
    {
        ...
        } else
        {
	        ...
	        else if(da<0) m_Unit[1].m_Angle-=props->max_da;
        ...
        if (dang > props->max_top_angle) dang = props->max_top_angle;
        else
        ...
        } else
        {
	        ...
	        else if(da<0) m_AngleX-=props->max_da;
	        else m_AngleX+=props->max_da;
        }
...
//                            D3DXVECTOR3(m_GeoCenter.x, m_GeoCenter.y, 20) + 100 * D3DXVECTOR3(cos(m_Unit[1].m_Angle), sin(m_Unit[1].m_Angle), 0));


}

bool CMatrixCannon::Damage(EWeapon weap, const D3DXVECTOR3 &pos, const D3DXVECTOR3 &, int attacker_side, CMatrixMapStatic* attaker)
{
    DTRACE();
...
inst_death:;
        ...
        CMatrixEffect::CreateSmoke(&m_Unit[0].m_Smoke, m_Unit[0].m_Pos, m_Unit[0].m_TTL + 100000, 1000, 100, 0xFF000000,false, 1.0f/30.0f);
        ...
        for (int i=1; i<m_UnitCnt; ++i)
        {
            ...
            CMatrixEffect::CreateSmoke(&m_Unit[i].m_Smoke, m_Unit[i].m_Pos, m_Unit[i].m_TTL + 100000, 1000, 100, 0xFF000000,false, 1.0f/30.0f);
        }
