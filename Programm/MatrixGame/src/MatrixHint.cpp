...
void    CMatrixHint::DrawNow(void)
{
    ...
    v[0].p = D3DXVECTOR4( float(m_PosX), float(m_PosY + szy), HINT_Z, 1.0f );
    v[1].p = D3DXVECTOR4( float(m_PosX), float(m_PosY), HINT_Z, 1.0f );
    v[2].p = D3DXVECTOR4( float(m_PosX + szx), float(m_PosY + szy), HINT_Z, 1.0f );
    v[3].p = D3DXVECTOR4( float(m_PosX + szx), float(m_PosY), HINT_Z, 1.0f );
...
void    CMatrixHint::DrawAll(void)
{
    ...
    for (;h;h = h->m_Next)
    {
        ...
        v[0].p = D3DXVECTOR4( float(h->m_PosX), float(h->m_PosY + szy), HINT_Z, 1.0f );
        v[1].p = D3DXVECTOR4( float(h->m_PosX), float(h->m_PosY), HINT_Z, 1.0f );
        v[2].p = D3DXVECTOR4( float(h->m_PosX + szx), float(h->m_PosY + szy), HINT_Z, 1.0f );
        v[3].p = D3DXVECTOR4( float(h->m_PosX + szx), float(h->m_PosY), HINT_Z, 1.0f );
