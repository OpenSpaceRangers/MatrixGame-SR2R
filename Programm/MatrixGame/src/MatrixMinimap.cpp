...
void CMinimap::SMMTex::Load(CBlockPar *mm, const wchar *name)
{
    ...
    if (tex == NULL)
    {
        const CWStr &n = mm->ParGet(name);
        tex = (CTextureManaged *)g_Cache->Get(cc_TextureManaged, n.GetStrPar(0,L","));
        tex->MipmapOff();
        tex->Preload();
...
{
    DTRACE();
    if (m_Texture == NULL || m_TextureStore != NULL) return;

    IDirect3DSurface9 *surf_to, *surf_from;
    if (D3D_OK != g_D3DD->CreateTexture(MINIMAP_SIZE, MINIMAP_SIZE, 1, 0, D3DFMT_X8R8G8B8, D3DPOOL_SYSTEMMEM, &m_TextureStore, NULL))
    {
        goto really_lost;
    }
...
void CMinimap::RestoreTexture(void)
{
    ...
    if (D3D_OK != g_D3DD->CreateTexture(MINIMAP_SIZE, MINIMAP_SIZE, 1, D3DUSAGE_RENDERTARGET, D3DFMT_X8R8G8B8, D3DPOOL_DEFAULT, &newTexture, NULL))
    {
        m_TextureStore->Release();
        m_TextureStore = NULL;
        return;
...
#ifdef MINIMAP_SUPPORT_ROTATION
    D3DXVec2TransformCoordArray((D3DXVECTOR2 *)&m_Verts[0].p, sizeof(SVert_V4_UV), (D3DXVECTOR2 *)&m_Verts[0].p, sizeof(SVert_V4_UV), &m_Rotation, 4);
#endif
...
void CMinimap::DrawOutIndicator(const D3DXVECTOR2 &in, float r, DWORD c1, DWORD c2, bool doclip, EMMTex tex)
{
    ...
    SVert_V4_UV  v[4];
    v[0].tu = m_Tex[tex].u0; v[0].tv = m_Tex[tex].v1;
    v[1].tu = m_Tex[tex].u0; v[1].tv = m_Tex[tex].v0;
    v[2].tu = m_Tex[tex].u1; v[2].tv = m_Tex[tex].v1;
    v[3].tu = m_Tex[tex].u1; v[3].tv = m_Tex[tex].v0;
...
{
    CMatrixMapStatic *ms = g_MatrixMap->GetPlayerSide()->GetArcadedObject();
    if (ms == NULL) return;
    float mapradius;

    if (ms->IsRobot())
    {
        mapradius = ms->AsRobot()->m_RadarRadius;
    } else 
#ifdef _DEBUG
        if (ms->IsFlyer())
#endif
    ...
    m._11 = scale * cosa;   m._12 = -scale * sina;    m._13 = 0;      m._14 = 0; 
    m._21 = scale * sina;   m._22 = scale * cosa;     m._23 = 0;      m._24 = 0; 
    m._31 = 0;              m._32 = 0;                m._33 = 0;      m._34 = 0; 
    m._43 = 0;      m._44 = 1; 

    m._41 = scale * (-g_MatrixMap->m_Camera.GetFrustumCenter().x*cosa - g_MatrixMap->m_Camera.GetFrustumCenter().y*sina) + x;
    m._42 = scale * (g_MatrixMap->m_Camera.GetFrustumCenter().x*sina - g_MatrixMap->m_Camera.GetFrustumCenter().y*cosa) + y;
    ...
    for (;ms;ms = ms->GetNextLogic())
    {
        ...
        switch (ms->GetObjectType())
        {
            case OBJECT_TYPE_ROBOTAI:
                ...
                if (r2 < D3DXVec2LengthSq(&(*(D3DXVECTOR2 *)&ms->GetGeoCenter() - *(D3DXVECTOR2 *)&g_MatrixMap->m_Camera.GetFrustumCenter())))
                {
                    continue;
                }
        ...
        //ASSERT_DX(g_D3DD->SetRenderState(D3DRS_TEXTUREFACTOR, (m_Color & 0xFF000000) | g_MatrixMap->GetSideColorMM(ms->GetSide())));
...
void CMinimap::Draw(void)
{
    ...
    ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MAGFILTER,			D3DTEXF_LINEAR));
	ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MINFILTER,			D3DTEXF_LINEAR));
	ASSERT_DX(g_D3DD->SetSamplerState(0,D3DSAMP_MIPFILTER,			D3DTEXF_NONE));
    ...
    for (int i=0; i<m_EventsCnt; ++i)
    {
        ...
        } else
        {
            ...
            v[0].tu = m_Tex[MMT_POINT].u0; v[0].tv = m_Tex[MMT_POINT].v1;
            v[1].tu = m_Tex[MMT_POINT].u0; v[1].tv = m_Tex[MMT_POINT].v0;
            v[2].tu = m_Tex[MMT_POINT].u1; v[2].tv = m_Tex[MMT_POINT].v1;
            v[3].tu = m_Tex[MMT_POINT].u1; v[3].tv = m_Tex[MMT_POINT].v0;
    ...
    //    D3DXVec2TransformCoordArray((D3DXVECTOR2 *)&v[0].p, sizeof(SMinimapVertex), (D3DXVECTOR2 *)&v[0].p, sizeof(SMinimapVertex), &m_Rotation, 4);
    //    ASSERT_DX(g_D3DD->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, &v, sizeof(SMinimapVertex) ));
    //}
    ...
    // right top <- left top
    if (g_MatrixMap->m_Camera.GetFrustumLT().z >= 0) {k = 100000;} else
    ...
    campts[0].z = MINIMAP_Z; campts[0].w = 1.0f;

    if (g_MatrixMap->m_Camera.GetFrustumRT().z >= 0) {k = 100000;} else
    ...
    campts[1].z = MINIMAP_Z; campts[1].w = 1.0f;

    if (g_MatrixMap->m_Camera.GetFrustumRB().z >= 0) {k = 100000;} else
    ...
    campts[2].z = MINIMAP_Z; campts[2].w = 1.0f;

    if (g_MatrixMap->m_Camera.GetFrustumLB().z >= 0) {k = 100000;} else
    ...
    campts[3].z = MINIMAP_Z; campts[3].w = 1.0f;
...
#ifdef MINIMAP_SUPPORT_ROTATION
    D3DXVec2TransformCoordArray((D3DXVECTOR2 *)&campts[0], sizeof(D3DXVECTOR4), (D3DXVECTOR2 *)&campts[0], sizeof(D3DXVECTOR4), &m_Rotation, 4);
#endif
...
void CMinimap::RenderBackground(const CWStr &name, DWORD uniq)
{
    ...
    CStr mmname(FOLDER_NAME_CACHE, g_CacheHeap);
    {
        mmname += "\\";
        mmname += CStr(name,g_CacheHeap);
        mmname += ".";

        mmname += (char)((uniq&0x0F)+65); uniq >>= 4;
        mmname += (char)((uniq&0x0F)+65); uniq >>= 4;
        mmname += (char)((uniq&0x0F)+65); uniq >>= 4;
        mmname += (char)((uniq&0x0F)+65); uniq >>= 4;
        mmname += (char)((uniq&0x0F)+65); uniq >>= 4;
...

    {
        ...
        if (ff != INVALID_HANDLE_VALUE)
        {
            ...
            if (bm.SizeX() != MINIMAP_SIZE || bm.SizeY() != MINIMAP_SIZE) goto render;

            LPDIRECT3DTEXTURE9 lt;
            ASSERT_DX(g_D3DD->CreateTexture(MINIMAP_SIZE, MINIMAP_SIZE, 1, 0, D3DFMT_X8R8G8B8, D3DPOOL_SYSTEMMEM, &lt, NULL));
...
render:
    ...
    if (CTerSurface::IsSurfacesPresent()) g_MatrixMap->BeforeDrawLandscapeSurfaces(true);
...
//	D3DXVec3Normalize((D3DXVECTOR3 *)(&(light.Direction)),(D3DXVECTOR3 *)(&(light.Direction)));
    ...
    if (CTerSurface::IsSurfacesPresent()) g_MatrixMap->DrawLandscapeSurfaces(true);
...
//	FAILED_DX(g_D3DD->SetDepthStencilSurface(oldZBuffer));
    ...
    {
        CreateDirectory(FOLDER_NAME_CACHE, NULL);

        // seek files

        CStr    n(FOLDER_NAME_CACHE,g_CacheHeap);
        n += "\\";
        ...
        {   
            ...
            if (ff != INVALID_HANDLE_VALUE)
            {
                CStr    nn(FOLDER_NAME_CACHE,g_CacheHeap);
                nn += "\\";
                nn += fd.cFileName;
                DeleteFile(nn.Get());
            } else
        ...
        ASSERT_DX(g_D3DD->CreateTexture(MINIMAP_SIZE, MINIMAP_SIZE, 1, 0, D3DFMT_X8R8G8B8, D3DPOOL_SYSTEMMEM, &out, NULL));
