...
void CTransition::BuildTexture(void)
{
    ...
    for (int i=0;i<m_GeomCnt;++i)
    {
        ...
        if (tra->ParGetName(i).CompareFirst(L"Up,")) m_Geom[i].dir.y = -1;
        else if (tra->ParGetName(i).CompareFirst(L"Down,")) m_Geom[i].dir.y = 1;
        else if (tra->ParGetName(i).CompareFirst(L"Left,")) m_Geom[i].dir.x = -1;
        else if (tra->ParGetName(i).CompareFirst(L"Right,")) m_Geom[i].dir.x = 1;
