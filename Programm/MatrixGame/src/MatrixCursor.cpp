...
void CMatrixCursor::Takt(int ms)
{
    ...
    while (m_Time > m_NextCursorTime)
    {
        ...
        {
            ++m_Frame;
            if (m_Frame >= m_FramesCnt) m_Frame = 0;
        } else
        {
            m_Frame += m_FrameInc;
            if (m_FrameInc > 0)
            {
                if (m_Frame >= m_FramesCnt) { m_Frame -= 2; m_FrameInc = -m_FrameInc; } 
            } else
            {
                if (m_Frame < 0) { m_Frame += 2; m_FrameInc = -m_FrameInc; } 
            }
...
void CMatrixCursor::Draw(void)
{
    ...
    if (g_Config.m_SoftwareCursor && FLAG(m_CursorFlags, CURSOR_VISIBLE))
    {
        ...
        v[0].tu = m_u0; v[0].tv = m_v1;
        v[1].tu = m_u0; v[1].tv = m_v0;
        v[2].tu = m_u1; v[2].tv = m_v1;
        v[3].tu = m_u1; v[3].tv = m_v0;
        ...
        v[0].p = D3DXVECTOR4( float(x0), float(y1), CURSOR_Z, 1.0f );
        v[1].p = D3DXVECTOR4( float(x0), float(y0), CURSOR_Z, 1.0f );
        v[2].p = D3DXVECTOR4( float(x1), float(y1), CURSOR_Z, 1.0f );
        v[3].p = D3DXVECTOR4( float(x1), float(y0), CURSOR_Z, 1.0f );
