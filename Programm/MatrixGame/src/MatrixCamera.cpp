...
void SAutoFlyData::Takt(float ms)
{
    ...
    {
        m_WarPairs[i].ttl -= ms;
        if (m_WarPairs[i].ttl < 0 || m_WarPairs[i].target->m_Object == NULL || m_WarPairs[i].attacker->m_Object == NULL)
        {
            m_WarPairs[i].target->Release();
            m_WarPairs[i].attacker->Release();
    ...
    if (m_CalcDestAngZTime < 0)
    {
        ...
        g_MatrixMap->FindObjects(*(D3DXVECTOR2 *)&m_Cur, 700, 1, TRACE_ROBOT|TRACE_CANNON, NULL, SeekCamObjects, (DWORD)&vecs);
    ...
    m_Speed += ms * 0.1f;
    if (m_Speed > len) m_Speed = len;

    if (m_Speed > 400) m_Speed = 400;

    float ilen = 0;
    if (len > 0.0001f) ilen = INVERT(len);
    ...
    if (m_ObzorTime > 0)
    {
        ...
        CMatrixMapStatic * ts = g_MatrixMap->Trace(NULL, g_MatrixMap->m_Camera.GetFrustumCenter(), 
    ...
    } else
    {
        ...
        if (m_RotSpeedX > 0.1f) m_RotSpeedX = 0.1f;
        if (m_RotSpeedX < -0.1f) m_RotSpeedX = -0.1f;
         

        m_CurAngX += m_RotSpeedX * ms * 0.001f;

        float z = g_MatrixMap->GetZ(m_Cur.x + TableSin(-m_CurAngZ) * m_CurDist, m_Cur.y + TableCos(-m_CurAngZ) * m_CurDist);

        m_NewAngX = GRAD2RAD(50) - (z * 0.002f) * GRAD2RAD(70);
        if (m_NewAngX < GRAD2RAD(10)) m_NewAngX = GRAD2RAD(10);
...
check_again:
            if (m_WarPairCurrent.target->m_Object && m_WarPairCurrent.attacker->m_Object)
            {
                m_New = (m_WarPairCurrent.target->m_Object->GetGeoCenter() + 
                    m_WarPairCurrent.attacker->m_Object->GetGeoCenter()) * 0.5f;
                return;
            }

            SObjectCore *seek_for = NULL;
            if (m_WarPairCurrent.target->m_Object) seek_for = m_WarPairCurrent.target;
            else if (m_WarPairCurrent.attacker->m_Object) seek_for = m_WarPairCurrent.attacker;
        ...
        if (m_WarPairsCnt == 0) goto seek_nothing;
...
void CMatrixCamera::CalcLinkPoint(D3DXVECTOR3 &lp, float &angz)
{
    ...
    DCP();
        ...
        lp.z = g_Config.m_CamParams[CAMERA_STRATEGY].m_CamHeight + g_MatrixMap->GetZInterpolatedLand(m_XY_Strategy.x,m_XY_Strategy.y);
...
//#endif
    ...
    DCP();
        ...
        if (m_ModeIndex == CAMERA_STRATEGY)
        {
            ...
            if (FLAG(m_Flags, CAM_ACTION_MOVE_LEFT)) { m_XY_Strategy += lDir; }
            if (FLAG(m_Flags, CAM_ACTION_MOVE_RIGHT)) { m_XY_Strategy += rDir; }
            if (FLAG(m_Flags, CAM_ACTION_MOVE_UP)) { m_XY_Strategy += tDir; }
            if (FLAG(m_Flags, CAM_ACTION_MOVE_DOWN)) { m_XY_Strategy += bDir; }
        ...
        {
            RESETFLAG(m_Flags, CAM_ACTION_MOVE_LEFT|CAM_ACTION_MOVE_RIGHT|CAM_ACTION_MOVE_UP|CAM_ACTION_MOVE_DOWN);
        }
    ...
    RESETFLAG(m_Flags, CAM_ACTION_ROT_LEFT|CAM_ACTION_ROT_RIGHT|CAM_ACTION_ROT_UP|CAM_ACTION_ROT_DOWN|CAM_ACTION_MOVE_LEFT|CAM_ACTION_MOVE_RIGHT|CAM_ACTION_MOVE_UP|CAM_ACTION_MOVE_DOWN);
}
