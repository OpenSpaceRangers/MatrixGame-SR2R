...
static int ZL03_UnCompress(CBuf &out, BYTE *in, int inlen)
{
    ...
    if (inlen < 8) return 0;
    if (*(in+0) != 'Z' || *(in+1) != 'L' || *(in+2) != '0' || *(in+3) != '3') return 0;
...
CDataBuf *CStorageRecord::GetBuf(const wchar *column, EStorageType st)
{
    for (int i=0; i<m_ItemsCount; ++i)
    {
        if (m_Items[i].GetName() == column) return m_Items[i].GetBuf(st);
    }
    return NULL;
}
...
bool CStorage::Load(CBuf &buf_in)
{
    ...
    if (tag != 0x47525453) return false;
