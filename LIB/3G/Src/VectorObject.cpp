...
void CVectorObject::Unload()
{
    ...
    if (m_Geometry.m_Vertices.verts != NULL)
    {
        ...
        if(m_Geometry.m_VerticesIdx != NULL) { HFree(m_Geometry.m_VerticesIdx, g_CacheHeap); }
        if(m_Geometry.m_FramesIdx != NULL) { HFree(m_Geometry.m_FramesIdx, g_CacheHeap); }
        if(m_Geometry.m_UnionsIdx != NULL) { HFree(m_Geometry.m_UnionsIdx, g_CacheHeap); }
        ...
        if(m_Geometry.m_Triangles != NULL) { HFree(m_Geometry.m_Triangles, g_CacheHeap); }
        ...
        if(m_Geometry.m_Unions != NULL) { HFree(m_Geometry.m_Unions, g_CacheHeap); }
        if(m_Geometry.m_Edges != NULL) { HFree(m_Geometry.m_Edges, g_CacheHeap); }
        if(m_Geometry.m_Frames != NULL) { HFree(m_Geometry.m_Frames, g_CacheHeap); }
        if(m_Geometry.m_FramesRuntime != NULL) { HFree(m_Geometry.m_FramesRuntime, g_CacheHeap); }
        ...
        if(m_Geometry.m_Surfaces != NULL) { HFree(m_Geometry.m_Surfaces, g_CacheHeap); }
        if(m_Geometry.m_Animations != NULL) { HFree(m_Geometry.m_Animations, g_CacheHeap); }
        if(m_Geometry.m_Matrixs != NULL) { HFree(m_Geometry.m_Matrixs, g_CacheHeap); }
        //if(m_Geometry.m_Normals != NULL) { HFree(m_Geometry.m_Normals, g_CacheHeap); }
        if(m_Geometry.m_AllMatrixs != NULL) { HFree(m_Geometry.m_AllMatrixs, g_CacheHeap); }
...
CTextureManaged* CVectorObject::CalcShadowTextureWOMat(int cnt, CVectorObjectAnim ** obj, const int * noframe, const D3DXMATRIX * wm, int texsize,CVOShadowCliper *cliper, CBaseTexture * tex_to_update)
{
    ...
    if (CustomZ)
    {
        ...
        oldZ->GetDesc(&d);
        g_D3DD->CreateDepthStencilSurface(texsize,texsize, d.Format, d.MultiSampleType, d.MultiSampleQuality, TRUE, &newZ, NULL);
    ...
    do
    {
        ...
        for (int surf = 0; surf < m_Geometry.m_SurfacesCnt; ++surf)
        {
            ...
            for(int i=0; i<k->m_UnionCnt; ++i)
            {
                ...
                if (gr->m_IBase < 0)
                {
                    res = g_D3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,gr->m_Base + vbase,0,gr->m_VerCnt,-gr->m_IBase + ibase, gr->m_TriCnt);
                } else
                {
                    res = g_D3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,gr->m_Base + vbase,gr->m_VerMinIndex,gr->m_VerCnt,gr->m_TriStart * 3 + ibase, gr->m_TriCnt);
                }
...
void CVectorObjectAnim::Init(CVectorObject * vo, CTextureManaged *tex_light, const SSkin *skin)
{
    ...
    if (tex_light) InitLights(tex_light);
}
        ...
        for (int m = 0;m<m_VO->GetMatrixCount(); ++m)
        {
            ...
            if (mn[0] == '$')
            {
                ...
                if (m_Lights[l].intervals_cnt == 0)
                {
                    ...
                    m_Lights[l].intervals[0].c1 = 0xFF000000 | li_info.GetStrPar(1, L",").GetStrPar(1, L":").GetHexUnsigned();
                ...
                m_Lights[l].BB().CBillboard::CBillboard(TRACE_PARAM_CALL D3DXVECTOR3(0,0,0), ra, 0, m_Lights[l].intervals[0].c1, tex_light);
                ++l;
            }
        }
    }
...
DWORD CVectorObjectAnim::NextFrame(void)
{
    ...
    {
        if (m_Frame < (fcnt-1)) m_Frame++;
        else return m_VO->GetAnimFrameTime(m_Anim, m_Frame);
    }
    m_VOFrame = m_VO->GetAnimFrameIndex(m_Anim,m_Frame);
...
CMain(),m_LinkMatrixName(g_CacheHeap),m_Name(g_CacheHeap)
{
	DTRACE();
...
CVectorObjectGroupUnit::~CVectorObjectGroupUnit()
{
	...
	if(m_ShadowStencil) { HDelete(CVOShadowStencil,m_ShadowStencil,g_CacheHeap); m_ShadowStencil=NULL; }
	...
	if(need & m_Flags & (VOUF_SHADOWSTENCIL)) {
		...
		if(!FLAG(m_Flags,VOUF_STENCIL_ON)) {
			...
			//if(m_Obj->VO()->EdgeExist())
            {
                ...
                m_ShadowStencil->Build(*(m_Obj->VO()),m_Obj->GetVOFrame(),light,(float)fabs(m_MatrixWorld._43 - m_Parent->m_GroundZ),false);
			}
		}
	}
}
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
CVectorObjectGroup::CVectorObjectGroup():CMain(),m_Name(g_CacheHeap), m_GroundZ(0)
{
	DTRACE();
	...
	m_ShadowStencilLight.x=0.0f; m_ShadowStencilLight.y=0.0f; m_ShadowStencilLight.z=-1.0f;
	//m_ShadowStencilCutPlane.a=0.0f;	m_ShadowStencilCutPlane.b=0.0f;	m_ShadowStencilCutPlane.c=1.0f; m_ShadowStencilCutPlane.d=0.0f;
...
void CVectorObjectGroup::BoundGet(D3DXVECTOR3 & bmin,D3DXVECTOR3 & bmax)
{
	...
	while(u) {
		if(u->m_Obj && u->m_Obj->VO())
        {
			...
			if(!ok) { ok=true; bmin=bminout; bmax=bmaxout; }
			else {
 ...
 //           m_Obj->SetAnimDefault();
	...
	while(gu) {
		if(!gu->m_LinkMatrixName.IsEmpty()) {
			if(gu->m_LinkMatrixName.GetCountPar(L",")!=2) ERROR_S4(L"In file: ",filename,L"   Unknown format link: ",gu->m_LinkMatrixName.Get());
			...
			if(gu->m_Link==NULL) ERROR_S4(L"In file: ",filename,L"   Not found link: ",tstr.Get());
