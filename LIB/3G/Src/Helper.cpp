...
void CHelper::Clear()
{
	...
	m_CntPrimitive=0;
	m_CntVertex=0;
	if(m_Vertex) { HFree(m_Vertex,g_CacheHeap); m_Vertex=NULL; }
	if(m_Index) { HFree(m_Index,g_CacheHeap); m_Index=NULL; }
}
...
void CHelper::Cone(D3DXVECTOR3 vFrom, D3DXVECTOR3 vTo, float rFrom, float rTo, DWORD colorFrom, DWORD colorTo, int seg_cnt)
{
    DTRACE();
	...
	for(i=0;i<seg_cnt;i++) {
        m_Index[index++]=i;
        m_Index[index]=i+1; if(m_Index[index]>=seg_cnt) m_Index[index]=0;
        index++;
        m_Index[index++]=i+seg_cnt;

        m_Index[index]=i+1; if(m_Index[index]>=seg_cnt) m_Index[index]=0;
        index++;
        m_Index[index]=i+1+seg_cnt; if(m_Index[index]>=(seg_cnt+seg_cnt)) m_Index[index]=seg_cnt;
        index++;
        m_Index[index]=i+seg_cnt;
        index++;
	}
...
void CHelper::ConeUpdate(D3DXVECTOR3 vFrom, D3DXVECTOR3 vTo, float rFrom, float rTo, DWORD colorFrom, DWORD colorTo)
{
	...
	if((m_CntVertex<6) || ((m_CntVertex & 1)==1)) return;
	...
	if(v.x>0) az=(float)atan(v.y/v.x);
    else if(v.x<0) az=pi+(float)atan(v.y/v.x);
    else if((v.x=0) && (v.y>=0)) az=pi/2.0f;
    else az=3.0f*pi/2.0f;
...
void CHelper::Draw()
{
	...
	if(m_CntIndex) {
		ASSERT_DX(g_D3DD->DrawIndexedPrimitiveUP(m_Type,0,m_CntVertex,m_CntPrimitive,m_Index,D3DFMT_INDEX16,m_Vertex,sizeof(SHelperVertex)));
	} else {
...
void CHelper::AfterDraw(void)
{
	...
	CHelper * he=m_First;
	while(he) {
        if(he->DeleteAfterCntDraw()==1) CHelper::AfterDraw_r(he);
		he=he->Next();
	}
	...
	while(he) {
		he2=he;
		he=he->Next();
        if(he2->DeleteAfterCntDraw()==1) CHelper::Destroy(he2);
	}

	he=m_First;
	while(he) {
		if(he->DeleteAfterCntDraw()>1) he->DeleteAfterCntDraw(he->DeleteAfterCntDraw()-1);
		he=he->Next();
	}
}
...
void CHelper::DestroyByGroup(int group)
{
	...
	while(he) {
		he2=he;
		he=he->Next();
        if(he2->Group()==group) CHelper::Destroy(he2);
	}
}
