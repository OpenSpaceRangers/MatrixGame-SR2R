...
//PURPOSE		: ѕровер€ет с какой стороны линии находитс€ точка
//RETURN VALUE	: True если справа или на линии, False в остальных случа€х
...
//PURPISE		:ѕровер€т попала ли точка внутрь 2D AABB
...
bool IntersectLine(const D3DXVECTOR2 & s1,const D3DXVECTOR2 & e1,const D3DXVECTOR2 & s2,const D3DXVECTOR2 & e2, D3DXVECTOR2 & out); // точка пересечени€=out
bool IntersectLine(const D3DXVECTOR2 & s1,const D3DXVECTOR2 & e1,const D3DXVECTOR2 & s2,const D3DXVECTOR2 & e2, float * t, float * u); // точка пересечени€=s1+(e1-s1)*t или s2+(e2-s2)*u.  ≈сли t>=0 and t<=1.0 то точка пересечени€ между s1 и e1

bool IsIntersectRect(float sx1,float sy1,float ex1,float ey1,float sx2,float sy2,float ex2,float ey2,float * sx,float * sy); // True - если два пр€моугольника пересекаютс€ sx,sy - направление и величина заступа
...
__forceinline double AngleNorm(double a) // Ќормировать угол. ¬озращает от -pi до +pi
{
...
__forceinline double AngleDist(double from,double to) // ƒистанци€ между углами. ¬озращает от -pi до +pi
{
...
// ZakkeR: „естно стырил из сырьков HL2
...
public:
    ...
    CTrajectory(CHeap *heap, D3DXVECTOR3 *points, int pcnt):m_Heap(heap),m_Len(-1),m_Segments(NULL), m_CurSegLen(-1) {Init2(points, pcnt); }   // init2 // точно
